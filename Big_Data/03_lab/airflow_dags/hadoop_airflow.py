...
with DAG(dag_id="popular-topics-hadoop", start_date=datetime(2025, 12, 6), schedule="@weekly", catchup=False) as dag:
    hadoop_mapreduce = BashOperator(task_id="hadoop-mapreduce", bash_command=f"{HADOOP_HOME}/bin/hadoop jar {HADOOP_STREAMING} -input {INPUT_FILE} -output {OUTPUT_DIR} -mapper 'python {SOURCE_DIR}/mapper.py' -reducer 'python {SOURCE_DIR}/reducer.py'")
